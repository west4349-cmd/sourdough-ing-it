...
type Save = { chapter: ChapterId; coins: number; owned: string[]; starterMade: boolean; starterFeeds: number; starterMature: boolean; };
...
export default function App(){
 ...
 const starterProgress=useMemo(()=>{let n=0;if(allSupplies)n++;if(game.starterMade)n++;if(game.starterFeeds>=1)n++;if(game.starterFeeds>=4)n++;if(game.starterMature)n++;return n},[allSupplies,game]);
 ...
 return <div className={`app-shell ${view==='town'?'town-mode':''} ${view==='quest'?'quest-mode':''}`}>
  ...
  <main className={view==='town'||view==='quest'?'town-main':''}>
   ...
   {view==='town'&&<section className="town-map town-master"><img src="./resources/bunnywood-town-master.png" alt="Bunnywood town"/>
    ...
    <button className="map-hotspot kitchen-hotspot" aria-label="Enter Recipe Kitchen" onClick={()=>goTownHotspot('kitchen')}/>
    <button className="map-hotspot grocery-hotspot" aria-label="Enter Grocery Store" onClick={()=>goTownHotspot('store')}/>
    <button className="map-hotspot quest-hotspot" aria-label="Enter Quest Meadow" onClick={()=>goTownHotspot('quest')}/>
